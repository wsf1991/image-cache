...
module.exports = class Core {
	...
	isFileExistInMem(pathFile) {
		try	{
			var a = memMap[pathFile];
	...
	fetchImageTwoLayers(image, callback) {

		if (image.existsInMem) {
			// image in mem
      console.log('previous mem: 1 -> now fetch from mem');
      var a = memMap[image.key];
      a.cacheMem = 'HIT';
			console.log(memMap[image.key]);
			callback(null, memMap[image.key]);
		}
		else {
			if (image.existsInFs) {
				this.readFileFetch({
					...
					if (error) {
						...
						console.log('previous mem: 0, fs: 1 -> now load file and put into mem');
						console.log(result);
						memMap[image.key] = result;
						callback(null, result);
					}
				});
			} else {
							...
							if (!result.error) {
								this.writeFileFetch({
									...
									if (error) {
										...
										console.log('previous mem: 0, fs: 0 -> now fetch file and put into fs and mem');
										console.log(result);
										memMap[image.key] = result;
										callback(null, result);
									}
		...
		async.waterfall([
			(callback) => {
				...
				async.each(images, (image, callbackEach) => {
					...
					let existsInMem = this.isFileExistInMem(fileName);
					let existsInFs = true;
					if (!existsInMem) {
						existsInFs = this.isFileExist(fileName);
					}
					console.log('img url:');
					console.log(image);
					console.log('existsInMem');
					console.log(existsInMem);
					console.log('existsInFs');
					console.log(existsInFs);
					...
					imagesMore.push({
						key: fileName,
						fileName: image,
						existsInFs: existsInMem,
						existsInMem: existsInMem,
						url: url
					});
			...
			(imagesMore, callback) => {
				async.map(imagesMore, this.fetchImageTwoLayers.bind(this), (error, results) => {
					if (error) {
						callback(error);
					} else {
